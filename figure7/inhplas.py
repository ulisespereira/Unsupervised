...
uI0=[np.array([0.1]) for i in range(npts)]
W0=[0.1*np.ones((n,n)) for i in range(npts)]
WEI0=[1.*np.ones(n) for i in range(npts)]

#integration
theintegrator=myintegrator(delay,dt,n,thetmax)
theintegrator.fast=False
u,uI,connectivity,WEI,t=theintegrator.DDE_Inh(field,x0,uI0,W0,WEI0,mystim.stim)
...
x0=0.01*np.ones((npts,n))
x0[:,0]=1.
uI0=[np.array([0.01]) for i in range(npts)]
W0=[connectivity[-1] for i in range(npts)]
WEI0=[WEI[-1] for i in range(npts)]

#integration
theintegrator=myintegrator(delay,dt,n,tmaxdyn)
theintegrator.fast=False
u_ret,uI_ret,connectivity_ret,WEI_ret,t_ret=theintegrator.DDE_Inh(field,x0,uI0,W0,WEI0,mystim.stim)
...
axLast.set_xlim([0,400])
axLast.set_xticks([0,200,400])
axLast.set_yticks([0.5,1])
axLast.set_xlabel('Time (ms)')
axLast.set_ylabel('Rate')
axLast.set_title('(C)',y=1.04)
