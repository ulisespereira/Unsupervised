...
def field(t,a,x_hist,W,H):
	...
	field_u=(1./tau)*(mystim.stim(t)+noise*amp_noise*np.sqrt(tau/dt)+conn_matrix.dot(phi(x_hist[-1],theta,uc))-x_hist[-1]-w_inh*np.dot(r1_matrix,phi(x_hist[-1],theta,uc)))#-a
	field_a=0.#in the paper we are not using adaptation during learning
	field_H=0.#(H*(1.-(post_u/y0))-H**2)/tau_H # not putting any homoestatic component.
	field_w=np.multiply(tauWinv(x_hist),winf(x_hist)-W)
	#print H,'holi'
	return field_a,field_u,field_w,field_H

def field_additive(t,x_hist,W):
	pre_u=x_hist[0]
	post_u=x_hist[-1]
	n=len(pre_u)
	conn_matrix=W
...
amp=3.2
amp_dc=-0.5#-3.3
amp_noise=0.2# amplitude of the noise
...
def tau_u0_theta(T):
	return -tau*np.log(1.-((thres-amp_dc)/(amp-amp_dc)))

def tau_umax_theta(T):
	return -tau*np.log(((thres-amp_dc)/(amp-amp_dc))*(1./(1.-np.exp(-T/tau)))) #including new stimulation
def tau_theta(T):
...
def recurrentTheo(T,k):
	...
	for i in range(k):
		...
		if myt-(delay+tau_u0_theta(T)+tstart)<=0:
			wk=0
		wdyn.append(wk)
	return wdyn
...
def feedforwardTheo(T,delta,k):
	...
	for i in range(k):
		...
		if myt-(tstart+tau_u0_theta(T))<=0:
			wk=0
		wdyn.append(wk)
	return wdyn
...
H0=[np.ones(n) for i in range(npts)] #not putting any homeostatic component
norbits=1
nperiod=2
mydelta=[8.,18.]
myT=[20.,36.]
...
#delta_T=[[55.,20.],[22.,8.5],[20.,15.],[30.,12.]] #parameters used in the paper
#delta_T=[[50.,50.],[22.,8.5],[20.,11.],[30.,10.]]
delta_T=[[50.,50.],[27.5,9.5],[31,10],[25,9]]
allRecurrent=[]
allFF=[]
allRecurrentTheo=[]
allFFTheo=[]
for param in delta_T:
	...
	tau_learning=400.
	row_sum,u,Wdiag,Woffdiag,connectivity,W01,t=theintegrator.DDE_Norm_additive(field_additive,x0,W0)
	...
	mystim.inten=0.
	mystim.amp_dc=0.
	tau_learning=30000. #learning rate hebbian learning rule
	thetmax=5000. #max time 
	theintegrator=myintegrator(delay,dt,n,thetmax)
	theintegrator.fast=False
...
#
	name='dynamics_'+str(elT)+'_'+str(eldelta)+'_amp_dc_'+str(amp_dc)+'.pdf'
	...
	name='dynamics_'+str(elT)+'_'+str(eldelta)+'_amp_dc_'+str(amp_dc)+'_zoom.pdf'
	plt.savefig(name, bbox_inches='tight')
	plt.close()
...
for i in range(1,n):
	...
	alph=alph+(0.95-0.15)/(n-1)
for i in range(1,n):
	myline1=np.linspace(w_i*(i+0.)/n,1,100)
	myconstant1=np.array([1+w_i*(i+0.)/n for l in range(0,100)])
	plt.fill_between(myline1,myconstant1,myconstant1+w_i/n,alpha=(1./(n-1))*i,edgecolor='grey', facecolor='green')
...
plt.savefig('bifdiagramTvsDel1_amp_dc_'+str(amp_dc)+'.pdf', bbox_inches='tight')
#plt.show()
