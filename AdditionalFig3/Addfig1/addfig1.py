...
def field(t,x_hist,W):
	...
	field_u=(1/tau)*(mystim.stim(t)+noise*amp_noise*np.sqrt(tau/dt)+W.dot(phi(x_hist[-1],theta,uc))-x_hist[-1]-w_inh*np.dot(r1_matrix,phi(x_hist[-1],theta,uc)))
...
for wsum in mywsum:
	...
	plt.xlabel('Time (s)')
	plt.ylabel('Rate')
#	plt.savefig('learning_stimall_sumrow_'+str(wsum)+'_delta_'+str(delta)+'_T_'+str(period)+'.png', bbox_inches='tight',dpi=600)
	#plt.show()
	plt.close()
